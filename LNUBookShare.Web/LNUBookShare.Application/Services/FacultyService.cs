...
namespace LNUBookShare.Application.Services
{
    public class FacultyService : IFacultyService
    {
        ...
        private readonly ILogger<FacultyService> _logger; // Додаємо логер

        public FacultyService(
            IFacultyRepository facultyRepository,
            IMemoryCache cache,
            IOptions<CacheSettings> cacheSettings,
            ILogger<FacultyService> logger) // Прокидаємо логер через конструктор
        {
        ...
        public async Task<Result<IEnumerable<Faculty>>> GetAllFacultiesAsync()
        {
            ...
            {
                // Блакитний (Cyan) для кешу - виглядає дуже "PRO"
